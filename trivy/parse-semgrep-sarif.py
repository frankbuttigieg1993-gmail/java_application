...
if __name__=="__main__":
    ...
    try : 
        with open(args.input, 'r') as f:
            data = json.load(f)
            # Accept both Semgrep JSON and SARIF 2.1.0 input.
            if 'results' in data:
                jdata = data['results']
                input_format = 'semgrep-json'
            elif 'runs' in data and data['runs']:
                run = data['runs'][0]
                driver = run.get('tool', {}).get('driver', {})
                rule_map = {r.get('id'): r for r in driver.get('rules', []) if r.get('id')}
                converted = []
                for finding in run.get('results', []):
                    ...
                    level = finding.get('level') or rule.get('defaultConfiguration', {}).get('level', 'warning')
                    severity = {'error': 'ERROR', 'warning': 'WARNING', 'note': 'INFO', 'none': 'UNKNOWN'}.get(level, str(level).upper())
                    props = rule.get('properties', {}) or {}
                    tags = props.get('tags', []) or []
                    confidence = next((x.split()[0] for x in tags if 'CONFIDENCE' in x), 'UNKNOWN')
                    cwes = [x for x in tags if x.startswith('CWE-')]
                    owasp = [x for x in tags if x.startswith('OWASP-')]
                    converted.append({
                        'check_id': rule_id,
                        'path': artifact.get('uri', ''),
                        'start': {'line': region.get('startLine', 0), 'col': region.get('startColumn', 0)},
                        'end': {'line': region.get('endLine', region.get('startLine', 0)), 'col': region.get('endColumn', region.get('startColumn', 0))},
                        'extra': {
                            'severity': severity,
                            'message': finding.get('message', {}).get('text', ''),
                            'lines': region.get('snippet', {}).get('text', ''),
                            'fix': None,
                            'metadata': {
                                'confidence': confidence,
                                'impact': 'UNKNOWN',
                                'likelihood': 'UNKNOWN',
                                'docs': rule.get('helpUri', ''),
                ...
                raise ValueError("Input is neither Semgrep JSON nor SARIF (missing results/runs)")
            ...
            output = t.render(
                ...
                scan_date=scan_date,
                version=(data.get('version') if input_format == 'semgrep-json' else data.get('runs', [{}])[0].get('tool', {}).get('driver', {}).get('semanticVersion', 'SARIF 2.1.0')),
                scanned_file_count=(len(data.get('paths', {}).get('scanned', [])) if input_format == 'semgrep-json' else len({x.get('path', '') for x in data.get('results', []) if x.get('path')})),
